-- get_person_detail's "completed" list (rendered as "Completed this
-- sprint" in team-pulse-54's people.tsx) had NO date scoping at all --
-- just the 10 most recently resolved tickets ever from
-- resolved_ticket_history, regardless of which sprint they were
-- actually resolved in. team-pulse-54 always calls this RPC with
-- p_sprint_start = null (see usePersonDetail there), so relying on that
-- param isn't an option; engineering-ethos passes a real value but only
-- 'current'/'upcoming' ever consulted it for anything.
--
-- Scoped to the ticket's OWN board's CURRENT, not-yet-ended sprint, not
-- one global cutoff or no cutoff at all -- boards run staggered sprint
-- cadences. Derives the board from the ticket's OWN jira_key prefix
-- (e.g. "TRG-1260" -> "TRG") rather than joining through
-- parent_epic_key -> epics: resolved_ticket_history's parent_epic_key is
-- whatever Jira's own `parent` field held, which is frequently a
-- non-Epic ticket that never has a row in `epics` at all -- that join
-- silently failed for most rows, tripping the "no match found" fallback
-- and letting stale entries straight through. The key-prefix is always
-- present and always correct. Falls back to showing everything when no
-- matching not-yet-ended sprint is found for that board, so a person
-- whose board isn't currently tracked doesn't go silently empty.
