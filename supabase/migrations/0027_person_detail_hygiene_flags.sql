-- Employee Details previously surfaced a fixed "70h available" capacity
-- number to engineers -- that figure is a backend planning constant, not
-- something an individual should be judged against. In its place the UI
-- now needs to show, per ticket, exactly what's missing: no worklog, no
-- comment, or no original estimate. get_person_detail's current/upcoming
-- ticket objects didn't expose worklog/comment presence, only the
-- estimate (already nullable) -- add the two missing booleans, and add
-- `updated` to `upcoming` so both lists can flag staleness consistently.
