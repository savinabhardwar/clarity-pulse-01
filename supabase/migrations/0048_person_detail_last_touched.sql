-- Exposes lastTouchedAt on `current` (in-progress) tickets: the most
-- recent worklog OR comment on that specific ticket, from ANY author --
-- not just whether one exists at all (hasWorklog/hasComment already
-- cover that), but WHEN the most recent one actually happened. Lets the
-- UI flag a ticket that technically has some old worklog/comment (so it
-- passes every existing hygiene check) but has otherwise sat untouched
-- for weeks -- found live: several people's WIP tickets went 3-8 weeks
-- without any update while the person stayed "compliant" because their
-- OTHER tickets were active. This is a purely informational tag, not a
-- new scoring input -- hygieneScore/computeJiraUpdateStatus are
-- unchanged.
