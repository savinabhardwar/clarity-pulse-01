-- Adds `loggedYesterday` (hours) to `current`/`upcoming` tickets -- lets the
-- People module surface "worked on yesterday" tickets at the top of Active
-- work with a tag showing how much was logged. "Yesterday" is the org's
-- real IST calendar day before today (same `at time zone 'Asia/Kolkata'`
-- convention as the sprint end-date fixes in 0056/0057), not a raw 24h
-- lookback -- consistent with how every other day-boundary check in this
-- app already works. Sums ALL worklogs on the ticket that day, no author
-- filter, matching `logged`'s own existing convention just above it.
