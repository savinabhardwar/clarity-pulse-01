-- `upcoming` (To Do) tickets never had a `logged` field -- only `current`
-- (In Progress) computed it. team-pulse-54's Active work table merges
-- current+upcoming into one shared "Logged" column, so a To Do row's
-- missing `logged` silently rendered as a blank "h" instead of "0h"
-- (found live, reported by the user: ACX-14/25/32 on Shreya Kumari's
-- card). Adds the same sprint-start-scoped computation `current` already
-- uses, so every row in that merged table has a real value.
